...
$output .= '<div'.$el_id.' class="wt-row-container'.$row_class.$el_class.$anim_class.'"'.$anim_data.$add_style .'>';
	...
	if ( $bg_type == 'youtube' && $youtube_video_id != '' ) {
		...
		$output .= '<a id="youtube-bg-video_'. $id .'" class="wt_youtube_player" data-property="{videoURL:\'https://www.youtube.com/watch?v='. $youtube_video_id .'\', containment:\'.wt-youtube-video-'. $id .'\', showControls:'.$yt_controls.', autoPlay:true, loop:true, mute:true, startAt:0, opacity:1, ratio:\'4/3\', addRaster:false, quality:\'default\'}"></a>';
		$output .= '</div>';
	}	
