...
class WPBakeryShortCode_WT_portfolio extends WPBakeryShortCode {
	...
	protected function content($atts, $content = null) {
		...
		if ($overlay_desc == "true") {
			...
			// If title, excerpt, category or read_more then the overlay bg should be black so they can be displayed with white color
			if ($title == 'true' || $excerpt == 'true' || $category == 'true' || $read_more == 'true') {
				$overlay_desc_css .= ' wt_portfolio_overlay_black';
			}
		} else {
		...
		if ($carousel == "true") {
			...
			$output .= '_carousel wt_portfolio-grid wt_portfolio_' . $columns . $overlay_desc_css . $grid_spaces_css . ' '.$css_class.'" style="margin-right: 0">';
			...
			$output .= '<div id="wt_owl_carousel_'.$id.'" class="wt_owl_carousel" data-owl-speed="600" data-owl-pagSpeed="1000" data-owl-autoPlay="'.$auto_slide.'" data-owl-stopOnHover="true" data-owl-navigation="false" data-owl-pagination="'.$carousel_nav.'" data-owl-items="4" data-owl-itemsDesktop="4"  data-owl-itemsSmallDesktop="3" data-owl-itemsTablet="3" data-owl-itemsMobile="2" data-owl-itemsMobileSmall="1">';
		}
		...
		while($wt_query->have_posts()) {
			...
			switch($columns){
				...
				case 2:	// portfolio two columns
					$width  = '730px';
					$height = '465px';
					$width_inline  = 'width="730"';
					$height_inline = 'height="654"';
					break;
				case 3: // portfolio three columns
					$width  = '730px';
					$height = '465px';
					$width_inline  = 'width="730"';
					$height_inline = 'height="465"';
					break;				
			...
			if (has_post_thumbnail()) {
				...
				$output .= '<img src="'. aq_resize( wt_get_image_src($image[0]), $width, $height, true ).'" alt="' . get_the_title() . '" '.$width_inline.' '.$height_inline.' />';
...
if (function_exists('wpb_map')) {
	...
	wpb_map( array(
		...
		'params' => array(
			...
			array(
				'type'          => 'dropdown',
				'heading'       => esc_html__('Portfolio Overlay', 'wt_vcsc'),
				'param_name'    => 'overlay_desc',
				'value' => array( 
					__('Yes', 'wt_vcsc')   => 'true',
					__('No', 'wt_vcsc')    => 'false',
				),
				'description'   => esc_html__('If selected, portfolio title / category / excerpt will overlay the featured image on hover.', 'wt_vcsc')
			),
			array(
				'type'          => 'textfield',
